<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="版块名称" prop="bankuaimingcheng">
          <el-input v-model="ruleForm.bankuaimingcheng" 
              placeholder="版块名称" clearable  :readonly="ro.bankuaimingcheng"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `bankuaimingcheng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.bankuaimingchengCrossAddOrUpdateFlag = false;
                  this.parent.search();
