<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="版块名称" prop="bankuaimingcheng">
          <el-select v-model="ruleForm.bankuaimingcheng" placeholder="请选择版块名称">
            <el-option
                v-for="(item,index) in bankuaimingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="版块名称" prop="bankuaimingcheng">
	      <el-input v-model="ruleForm.bankuaimingcheng"
                placeholder="版块名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.biaoti = true;
          ...
          if(o=='bankuaimingcheng'){
            this.ruleForm.bankuaimingcheng = obj[o];
	    this.ro.bankuaimingcheng = true;
            continue;
          }
          if(o=='neirong'){
      ...
      this.$http({
		...
		if(json.touxiang!=''&&json.touxiang){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/bankuaimingcheng/bankuaimingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.bankuaimingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssm5848t/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhubankuai/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhubankuaiCrossAddOrUpdateFlag = false;
                  this.parent.search();
